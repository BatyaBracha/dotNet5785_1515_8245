...
public static class Initialization
{
    ...
    {
        int[] validIsraeliIds =  { 218853810, 236195475, 555762350, 977675206, 412935462, 741382543, 750319766, 414983635, 389204397, 905626214, 892208588, 816012173, 896314481 }; string[] names = { "Tzipy", "Chani", "Yehudit", "Shira", "Moishy", "Efraim", "Shloimy", "Yosi", "Yoni", "Avi", "Chaim", "Ester", "Rachel", "Shani", "Michali" };
  ...
  (31.781869,  35.236349 ),(  31.799467,  35.240458 ), (  31.788469,  35.235679 )};
        ...
        s_dal!.Volunteer.Create(new(328118245, "Mali Horowitz", "0583278625", "mh078625@gmail.com", "password", "Ha'ayalot 25", 31.8, 35.1, Role.ADMINISTRATOR, true, 250, (TypeOfDistance)s_rand.Next(0, 2)));

        for (int i = 0; i < 15; i++)
        {
            s_dal!.Volunteer.Create(new(validIsraeliIds[i], names[i], phones[i], $"{names[i]}{phones[i]}@gmail.com", passwords[i], addresses[i], coordinates[i].Latitude, coordinates[i].Longitude, Role.STANDARD, true, maxDistances[i], (TypeOfDistance)s_rand.Next(0, 2)));
        }
